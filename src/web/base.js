...
class ZExpireStore {
    ...
    isExpired() {
        let res = this.expiredStorage.isExpired(this.storagename) || this.expiredStorage.getItem(this.storagename) != this.defval;
        return res
    }
    setExpire(minute = 3) {
        this.expiredStorage.setItem( this.storagename, this.defval, 60 * minute)
    }
    clearExpired() {
        this.expiredStorage.clearExpired()
    }
}

window.ZExpireStore = ZExpireStore